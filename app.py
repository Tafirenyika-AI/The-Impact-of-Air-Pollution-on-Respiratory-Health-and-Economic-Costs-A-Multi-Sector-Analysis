# =============================================================================
# Streamlit‑ready version (with RMSE fix)
# =============================================================================
...
# LOAD DATASETS
# ----------------------------
pollution_df = pd.read_csv("airq2.csv")
economic_df  = pd.read_csv("data1.csv")
health_df    = pd.read_csv("res.csv")

# ----------------------------
# CLEANING DATASETS
# ----------------------------
pollution_df = (
    pollution_df[["Country", "PM2.5 AQI Value"]]
    .dropna()
    .groupby("Country")
    .mean()
    .reset_index()
...
years = ["2017 [YR2017]", "2018 [YR2018]", "2019 [YR2019]",
...
economic_df.iloc[:, 1:] = economic_df.iloc[:, 1:].apply(
    pd.to_numeric, errors="coerce"
)

health_df["season"] = health_df["season"].astype(str)
health_summary = (
    health_df.groupby("season")["percent"]
    .mean()
    .reset_index()
    .rename(columns={"percent": "Avg_Respiratory_Visit_Percent",
                     "season": "Year"})
)
health_summary["Year"] = health_summary["Year"].str[:4]
health_summary = (
    health_summary[health_summary["Year"].str.isnumeric()]
    .astype({"Year": int})
)
...
merged_df["Avg_Health_Expenditure"] = merged_df[
    ["2017", "2018", "2019", "2020", "2021"]
].mean(axis=1)

merged_df = merged_df[[
    "Country", "PM2.5 AQI Value", "Avg_Health_Expenditure"
]].dropna()
...
st.write(merged_df[["PM2.5 AQI Value", "Avg_Health_Expenditure"]].corr())
...
sns.lineplot(data=health_summary,
             x="Year",
             y="Avg_Respiratory_Visit_Percent",
             marker="o")
...
expenditure_over_time = (
    economic_df.drop(columns=["Country"])
    .set_index(economic_df["Country"])
    .dropna()
)
